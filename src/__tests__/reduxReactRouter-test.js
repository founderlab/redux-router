...
describe("reduxRouter()", () => {
  ...
  it("works with navigation action creators", () => {
    ...
    store.dispatch(
      replace({ pathname: "/parent/child/321", query: { key: "value2" } })
    );
    expect(store.getState().router.location.pathname).to.equal(
      "/parent/child/321"
    );
  ...
  it("handles async middleware", done => {
    ...
    compose(
      ...
      applyMiddleware(() => next => action => setTimeout(() => next(action), 0))
    )(createStore)(reducer);
  ...
  it("accepts history object when using basename", () => {
    ...
    expect(store.getState().router.location.pathname).to.eql(
      "/parent/child/123"
    );
